(function ($, window) {
     ...
     */
    App.Model.News = Backbone.Model.extend({
        defaults : {
            translated_en: null,
            is_draft: null,
            title: null,
    ...
    App.Collection.AllNews = Backbone.Collection.extend({
        model: App.Model.News
    });

    var allNews = new App.Collection.AllNews();
    ...
    App.Collection.News = Backbone.Collection.extend({
        ...
        parse: function (response) {
            ...
            allNews.add(response.list.data);
    ...
    var news = new App.Collection.News({language: 'en'});

    /**
     * Представление списка новостей
     */
    App.View.News = new Vue({
        el: "#news",
        ...
        mounted: function () {
            ...
            this.news.fetch({language: this.language, data: { created_at:  this.filter}});
        },
        updated: function () {
        ...
        computed: {
            hasMoreNews: function () {
                var pageInfo = this.news.pageInfo({language: this.language});
                return !(pageInfo.total <= Math.ceil(pageInfo.page * pageInfo.perPage));
            }
        },
        methods: {
            loadMore: function () {
                this.news.nextPage({language: this.language, data: { created_at:  this.filter}, success: this.appendCard});
            },
            appendCard: function () {
                var self = this;
                this.news.each(function (model) {
                    self.newsList.push({
                        id: model.get('id'),
                        url: ((self.language === 'en') ? '/en/' : '/') + 'news/' + model.get('url'),
            ...
            showSingle: function (id) {
                var model = this.allNews.get(id);
                if (model === undefined){
                    return;
                }
                this.singleNews = {
                    id: model.get('id'),
                    url: '/news/' + model.get('url'),
