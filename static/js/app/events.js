(function ($, window) {
    ...
    Vue.component('events-card', {
        props: ['id', 'url', 'image', 'title', 'date', 'description', 'single'],
        template: '#events-card'
    });

    Vue.component('events-single-card', {
        props: ['url', 'image', 'title', 'date', 'description', 'text', 'all'],
        template: '#events-single-card'
    });

    /**
     * Модель списка новостей
     */
    App.Model.events = Backbone.Model.extend({
        defaults : {
            translated_en: null,
            is_draft: null,
            title: null,
    ...
    App.Collection.Allevents = Backbone.Collection.extend({
        model: App.Model.events
    });

    var allevents = new App.Collection.Allevents();
    ...
    App.Collection.events = Backbone.Collection.extend({
        ...
        parse: function (response) {
            ...
            allevents.add(response.list.data);
    ...
    var events = new App.Collection.events({language: 'en'});

    /**
     * Представление списка новостей
     */
    App.View.events = new Vue({
        el: "#events",
        ...
        mounted: function () {
            ...
            this.events.fetch({language: this.language, data: { created_at:  this.filter}});
        },
        updated: function () {
        ...
        computed: {
            hasMoreevents: function () {
                var pageInfo = this.events.pageInfo({language: this.language});
                return !(pageInfo.total <= Math.ceil(pageInfo.page * pageInfo.perPage));
            }
        },
        methods: {
            loadMore: function () {
                this.events.nextPage({language: this.language, data: { created_at:  this.filter}, success: this.appendCard});
            },
            appendCard: function () {
                var self = this;
                this.events.each(function (model) {
                    self.eventsList.push({
                        id: model.get('id'),
                        url: ((self.language === 'en') ? '/en/' : '/') + 'events/' + model.get('url'),
            ...
            showSingle: function (id) {
                var model = this.allevents.get(id);
                if (model === undefined){
                    return;
                }
                this.singleevents = {
                    id: model.get('id'),
                    url: '/events/' + model.get('url'),
